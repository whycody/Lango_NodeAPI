...
describe('getSuggestionsForUser', () => {
    ...
    it('generates suggestions synchronously when active suggestions are below threshold', async () => {
        const suggestions = Array.from({ length: 5 }, (_, i) => makeSuggestion({ _id: `s${i}` }));
    ...
    it('returns existing suggestions without awaiting generation when enough active suggestions exist', async () => {
        ...
        const result = await getSuggestionsForUser(userId, mainLang, translationLang);
    ...
    it('filters by since parameter when enough suggestions exist', async () => {
        ...
        const result = await getSuggestionsForUser(userId, mainLang, translationLang, '2026-03-01');

        expect(result.every((s: any) => new Date(s.updatedAt) > new Date('2026-03-01'))).toBe(true);
    });
